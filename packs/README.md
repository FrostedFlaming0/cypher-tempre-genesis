...
| [recall-discipline](recall-discipline-v1.json) | 1.0 | long-horizon conversational memory | 6 senses + 6 modalities | forged from sealed recall failure modes: aggregate completeness, quantity clauses, temporal anchors, latest-value drift, trap premises, scope boundaries; scope filtering, cross-session fusion, term-table arithmetic, timeline construction, lineage resolution, coverage accounting |
| [recall-discipline](recall-discipline-v1.1.json) | 1.1 | long-horizon conversational memory | +4 senses, +2 modalities over v1.0 | earned in single-core long-horizon field use: variant drift (near-duplicate questions can hide different facts), list-position discipline, role-source routing, provenance-of-assertion (self-report vs pasted vs assistant); event-identity reconciliation (drifting re-mentions count once), answer-citation discipline (no span, no assertion) |
