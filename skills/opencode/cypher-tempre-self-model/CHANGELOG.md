...
chain-isolation warning — one chain per agent identity, fresh genesis, never
point `CT_OC_SKILL_DIR` at another agent's install — and cross-references the
...
into this fork. All upstream additions were kept (dormant faculty tier, relevance
retrieval over the dormant pool, per-turn wake in the labeler, reinstatement by use,
wake-first growth dedup, in-process auto-maintenance, static-scan regression fixes).
Conflict resolutions:

- `recall_cli.py`: the fork's restructured `cmd_turn` (pre-seal `_turn_growth`,
  enforced `_op_need_check`) is kept; v3.16's dormant-retrieval accounting
  (`note_retrieval`) is grafted in after the seal, and its "woke dormant faculties"
  reporting is ported into `_turn_growth` (wake-first dedup). v3.16's in-process
  `_auto_maintenance` rewrite is taken (drops the subprocess spawn).
- `tests/selftest.py`: both suites kept — the fork's phase19 (composability) /
  phase20 (fusion + autoexec) / bundle-equivalence, plus v3.16's hibernation suite
  (relabeled phase21).
- `CHANGELOG.md`: both sections retained.

Note: v3.16.0 does NOT change `immune.py` — the three-layer analyst-ring
false-positive class reported to upstream remains open.

### Merged upstream v3.15.0 — 2026-07-03

Merged upstream `main` (v3.15.0, "circulation") into this fork. All upstream
additions were kept (hippocampus auto-index on seal, PoQ entity-level grounding,
gate discrimination battery + saturation check, conjecture due-rings,
depth-completing governor, behavioral faculty payloads, effect-gated rent,
calibrators registry, embed auto-tier, recall physical split). Conflict
resolutions:

- `recall.py`: upstream moved the CLI into the new `recall_cli.py`; the fork's
  CLI deltas (pre-seal `_turn_growth` fusion loop, auto-compose/auto-pipeline,
  the enforced AUTHOR-OP trigger with `--computed-need`/`--skip-op-reason`)
  were ported into `recall_cli.py`; the fork's engine changes (composite DAG in
  `label()`) stay in `recall.py`.
- `enforce.py`: upstream's lazy `_max_nudges()` (calibrators-owned) replaces the
  fork's constant; the fork's `PROMPT_RECALL_*` constants are kept beside it.
- `cambium.py`: upstream's `effect` subparser lands alongside the fork's
  `autoexec`/`compose`.

Note: v3.15.0 does NOT change `immune.py` — the three-layer analyst-ring
false-positive class reported to upstream remains open.
