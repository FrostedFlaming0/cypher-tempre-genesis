...
def cmd_mark(_data):
    ...
    # Process-data layer: stamp the harness session pointer at turn start so the
    # seal can bind this turn's ring to its trajectory slice (bind, don't copy).
    # line_start = the transcript's current length, so the exporter slices only
    # THIS turn's tool events. Fail-open: no harness context, no stamp.
    st.pop("turn_trajectory", None)
    try:
        tp = _data.get("transcript_path") or os.environ.get("CT_SESSION_FILE")
        if tp and Path(tp).exists():
            with open(tp, "rb") as _tf:
                n_lines = sum(1 for _ in _tf)
            st["turn_trajectory"] = {
                "session_file": str(tp),
                "session_id": _data.get("session_id") or os.environ.get("CT_SESSION_ID"),
                "line_start": n_lines,
            }
    except Exception:
        pass
