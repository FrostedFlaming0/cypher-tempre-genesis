...
def cmd_audit(args):
    ...
    _print_verdict(v)
    # CT_CAPTURE_REJECTS (process-data layer): an audited candidate the gate
    # refuses on QUALITY grounds is a training reject — buffer it so the next
    # successful seal THIS turn harvests it as a preference pair. NO-LAUNDER:
    # a covenant/consistency REJECT is never buffered.
    if v["decision"] in ("REVISE", "FORCE_UNCERTAINTY"):
        try:
            import training as _training
            if _training.capture_enabled():
                ok = _training.pending_append(
                    args.root, _training.reject_entry(args.candidate, verdict=v, source="audit"),
                    _training.current_turn_head(args.root))
                if ok:
                    print("  [capture] refused candidate buffered — the next seal this turn "
                          "records it as a quality reject (chain/pending_rejects.json).")
        except Exception:
            pass
