...
def block_text(ring) -> str:
    ...
    # Process-data metadata (rejected candidates, verification, trajectory
    # pointers) is fetch-only evidence: it must never reach the grep/retrieve/
    # label text surface, or refused content would pollute recall (no-noise
    # guarantee of the CT_CAPTURE_REJECTS layer).
    payload = {k: v for k, v in payload.items()
               if k not in ("labels", "state", "poq_verdict",
                            "rejected", "verification", "trajectory_ref")}
...
class Recall:
    ...
    def seal(self, ring_type, summary, context="", external_scores=None, difficulty=0, files=None,
             ...
             at_risk=None, frame=None, stored_input=None,
             rejected=None, verification=None, trajectory_ref=None):
        """`stored_input` (fork): a screened, redacted copy of the request this turn
        answered, persisted as payload.input beside payload.summary — the instruction
        half of the instruction-response pair. It rides the extra_payload seam, which
        merges into the payload AFTER the gate evaluates, so it never influences the
        PoQ verdict.
        ...
        extra = {"labels": labels}
        if stored_input:
            extra["input"] = str(stored_input)
        # Persist the DECLARED credit assignment on the ring itself (v3.29.1).
        # Before this, --used-rings fed the relevance window and `use` telemetry
        # but the declared list never reached the payload — so no export (and no
        # future reader) could see what evidence the model said it relied on.
        if used_rings:
            extra["used_rings"] = [int(i) for i in used_rings]
        # Process-data layer (fork): rejected candidates, provenance-stamped
        # verification, and the harness trajectory pointer ride the same
        # post-gate extra_payload seam as `input` — they never influence the
        # PoQ verdict, and block_text() keeps them off the recall text surface.
        if rejected:
            extra["rejected"] = list(rejected)[:8]
        if verification:
            extra["verification"] = dict(verification)
        if trajectory_ref:
            extra["trajectory_ref"] = dict(trajectory_ref)
        if at_risk:
