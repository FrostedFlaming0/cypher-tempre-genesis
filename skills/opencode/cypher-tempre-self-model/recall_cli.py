...
from poq import tokens, clamp, POQ_WINDOW                            # noqa: E402
...
def _loop_seal(root, reg, ring_type, summary, context="", external_scores=None,
               used_rings=None, at_risk=None, frame=None, stored_input=None,
               rejected=None, verification=None, trajectory_ref=None):
    ...
    `stored_input` (fork): the screened request to persist as payload.input; it rides
    the honest seal and the uncertainty-led reseal (both are honest turns), but NEVER
    the REJECT refusal record (no-launder: a refusal record carries no request payload).
    `rejected`/`verification`/`trajectory_ref` (process-data layer): ride the same two
    honest paths and are likewise stripped from a REJECT refusal record; when
    CT_CAPTURE_REJECTS is on, the uncertainty-led reseal CAPTURES the original
    confident form as a quality-reject entry — the automatic preference pair.
    ...
    verdict, ring, labels = Recall(root, reg).seal(
        ...
        used_rings=used_rings, at_risk=at_risk, frame=frame, stored_input=stored_input,
        rejected=rejected, verification=verification, trajectory_ref=trajectory_ref)
    ...
    if verdict.get("decision") == "REJECT":
        ...
        # it is a refusal record, not the offending claim. NO-LAUNDER: the refusal
        # record carries NO request payload, NO rejected candidates, NO process data —
        # a covenant-refused candidate is never persisted, not even as a "reject".
    ...
    # Quality refusal (REVISE / FORCE_UNCERTAINTY): capture the confident form +
    # its first verdict as a reject entry on the reseal — chosen vs rejected with
    # PoQ margins, recorded as a side effect of honest operation.
    if _env_on("CT_CAPTURE_REJECTS", default="0"):
        try:
            import training as _training
            rejected = list(rejected or []) + [_training.reject_entry(
                summary, verdict=verdict, source="gate-reseal")]
        except Exception:
            pass   # capture is telemetry-grade: never block the honest reseal
    verdict, ring, labels = Recall(root, reg).seal(
        ring_type, _uncertainty_led(summary), context=context,
        external_scores=_UNCERTAIN_SCORES, at_risk=at_risk, stored_input=stored_input,
        rejected=rejected, verification=verification, trajectory_ref=trajectory_ref)
    return verdict, ring, labels, True
...
def cmd_turn(args):
    ...
    # 5b. COVENANT CONFRONTATION (v3.28) — the forced re-grounding. Before sealing, the
    ...
    # 5c. Persist the SCREENED request into the sealed ring (fork), so a turn ring
    #     carries the full instruction-response pair (payload.input beside
    #     payload.summary). It merges into the payload AFTER the gate evaluates, so
    #     it never influences the PoQ verdict. Discipline: only a CLEAN screen
    #     persists — tainted-admitted input stays DATA (no durable recall surface
    #     for an injection) and an unscreened input is never presumed clean.
    #     Secrets are masked with the same redactor Continuum applies to source.
    #     CT_STORE_INPUT=0 opts out; CT_STORE_INPUT_MAX caps length (default 4000).
    stored_input = None
    if args.input and input_screened and input_tainted is None and _env_on("CT_STORE_INPUT"):
        import os as _os_si
        try:
            from continuum import redact_secrets as _redact_si
            stored_input, _ = _redact_si(args.input)
        except Exception:
            stored_input = args.input
        try:
            _cap = int(_os_si.environ.get("CT_STORE_INPUT_MAX", "4000"))
        except ValueError:
            _cap = 4000
        stored_input = stored_input[:max(200, _cap)]
    # 5d. Process-data layer (fork): assemble rejected candidates (explicit file +
    #     the pending cross-invocation buffer), provenance-stamped verification,
    #     and the harness trajectory pointer. All fail-open — capture never
    #     blocks a seal. NO-LAUNDER discipline lives in _loop_seal.
    rejected, verification, trajectory_ref = [], None, None
    try:
        import training as _training
        if getattr(args, "rejected_file", None):
            raw = json.loads(Path(args.rejected_file).read_text())
            for item in (raw if isinstance(raw, list) else [raw]):
                rejected.append(_training.reject_entry(
                    item.get("candidate", ""), source="explicit",
                    tests=item.get("tests"),
                    verdict={"decision": item.get("verdict"),
                             "reasons": item.get("reasons") or [],
                             "scores": item.get("scores") or {}}
                    if item.get("verdict") != "REJECT" else None))
        if _training.capture_enabled():
            rejected += _training.pending_harvest(root, _training.current_turn_head(root))
        vf = getattr(args, "verification_file", None)
        if vf:
            verification = json.loads(Path(vf).read_text())
        elif getattr(args, "verification", None):
            verification = json.loads(args.verification)
        if verification and verification.get("provenance") != "harness":
            # a verification the wrapper did not produce is a CLAIM, and is
            # stamped as such so the exporter can filter it from training data
            verification["provenance"] = "self-report"
        trajectory_ref = _training.turn_trajectory_ref(root)
        if rejected:
            print(f"captured {len(rejected)} rejected candidate(s) on this ring "
                  f"(fetch-only; excluded from recall surface).")
    except Exception as _pd_exc:
        print(f"process-data capture: fail-open ({_pd_exc}) — sealing without it.")
        rejected, verification, trajectory_ref = rejected or [], verification, trajectory_ref
    ...
    verdict, ring, labels, reseal = _loop_seal(
        ...
        frame=getattr(args, "frame", None), stored_input=stored_input,
        rejected=rejected or None, verification=verification,
...
def cmd_fetch(args):
    ...
    for i in args.ids:
        ...
        if getattr(args, "with_rejects", False):
            payload = r.get("payload") or {}
            for n, rej in enumerate(payload.get("rejected") or []):
                print(f"  [REJECTED candidate {n + 1} — did NOT seal; "
                      f"verdict={rej.get('verdict', '?')} source={rej.get('source', '?')}]")
                print(f"    {str(rej.get('candidate', ''))[:400]}")
                if rej.get("reasons"):
                    print(f"    reasons: {'; '.join(rej['reasons'])[:200]}")
            v = payload.get("verification")
            if v:
                print(f"  [verification: provenance={v.get('provenance', '?')} "
                      f"exit={v.get('exit_code', '?')} summary={v.get('summary')}]")
...
def build_parser():
    ...
    pt.add_argument("--rejected-file", default=None,
                    help="JSON file of failed attempts to persist as payload.rejected "
                         "(a candidate that failed tests, with its result attached) — "
                         "fetch-only, excluded from the recall surface; REJECT-verdict "
                         "entries are refused (no-launder)")
    pt.add_argument("--verification", default=None,
                    help="inline verification JSON — stamped provenance:self-report "
                         "(the exporter filters it; prefer training.py run-verified)")
    pt.add_argument("--verification-file", default=None,
                    help="verification JSON written by training.py run-verified "
                         "(provenance:harness — measured, not claimed)")
    ...
    pf.add_argument("--with-rejects", action="store_true",
                    help="also render payload.rejected / payload.verification "
                         "(hidden by default: refused content is labeled evidence, "
                         "never grounding — do not treat it as an accepted claim)")
